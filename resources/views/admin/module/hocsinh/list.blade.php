          ...
          <div class="row">
            <div class="col-xs-12">
              ...
              <table style="border: 5px;">
               ...
               @php
               $nam = App\Models\Hocsinh::select()->where('lophoc_id', $lop['id'])->where('gioitinh',0)->count('id');
              $nu = App\Models\Hocsinh::select()->where('lophoc_id', $lop['id'])->where('gioitinh',1)->count('id');
              $sum = App\Models\Hocsinh::select()->where('lophoc_id', $lop['id'])->count('id');
               @endphp
               <tbody>
                
                 <tr><td>Nam:{{$nam}} Trên tổng số: {{$sum}} HS</td><td>Tỷ lệ: {{number_format(($nam *100/$sum),2)}}%</td></tr>
                 <tr><td>Nữ:{{$nu}} Trên tổng số: {{$sum}} HS</td><td>Tỷ lệ: {{number_format(($nu *100/$sum),2)}}%</td></tr>
